          ...
          <section>
            ...
            {general.operation_description && (
              <div className="mt-2 border border-gray-200 rounded p-3">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 mb-1">Operation Description</p>
                <p className="text-xs text-gray-800 whitespace-pre-wrap">{general.operation_description}</p>
              </div>
            )}
            {general.work_holding_notes && (
              <div className="mt-2 border border-gray-200 rounded p-3">
            ...
            {tools.length > 0 && (
              <section>
                ...
                <table className="print-table w-full">
                  <thead>
                    <tr>
                      {TOOL_COLS.map((c) => <th key={c}>{c}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {tools.map((tool, i) => (
                      <tr key={i}>
                        {TOOL_KEYS.map((k) => <td key={k}>{tool[k]}</td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {operations.length > 0 && (
            <section>
              ...
              <table className="print-table w-full">
                <thead>
                  <tr>
                    {OP_COLS.map((c) => <th key={c}>{c}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {operations.map((op, i) => (
                    <tr key={i}>
                      {OP_KEYS.map((k) => <td key={k}>{op[k]}</td>)}
                    </tr>
                  ))}
                </tbody>
              </table>
