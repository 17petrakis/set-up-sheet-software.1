...
function Label({ children }) {
  return <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider mb-0.5 block">{children}</span>;
}
...
function SmallSelect({ value, onChange, options, placeholder = "—", className = "w-20", allowOther = true }) {
  ...
  return (
    <Select value={value || undefined} onValueChange={(v) => {
      ...
      <SelectTrigger className={`h-9 text-sm px-2 [&>span]:line-clamp-none ${className}`} style={{ width: "fit-content", minWidth: "3.5rem" }}>
  ...
  return (
    <div className="border border-border/40 rounded-lg mb-2">
      ...
      <div className="flex items-start gap-2 px-3 py-2 bg-muted/20 hover:bg-muted/30 transition-colors flex-wrap">
        <button type="button" onClick={() => setExpanded(e => !e)} className="mt-6 text-muted-foreground hover:text-foreground shrink-0">
        ...
        {/* Kind badge */}
        {tool.tool_kind && (
          <span className={`mt-6 text-[11px] font-semibold px-2 py-0.5 rounded shrink-0 ${isMill ? "bg-blue-100 text-blue-700" : "bg-orange-100 text-orange-700"}`}>
            {tool.tool_kind}
          </span>
        )}
        ...
        {/* Tool Name — wraps to its own line if longer than the reference string */}
        <div className={`flex flex-col min-w-[120px] ${toolName.length > 27 ? "w-full" : "flex-1"}`}>
        ...
        <Button type="button" size="icon" variant="ghost" onClick={() => setConfirmDelete(true)}
          className="mt-6 h-9 w-9 text-destructive hover:text-destructive shrink-0">
        ...
        <div className="px-4 py-3 bg-background border-t border-border/30">
          <div className="flex flex-wrap items-start gap-x-3 gap-y-2">
            ...
            <Button type="button" size="icon" variant="ghost" onClick={() => setShowEditModal(true)}
              className="mt-6 h-9 w-9 text-muted-foreground hover:text-foreground shrink-0" title="Edit fields">
